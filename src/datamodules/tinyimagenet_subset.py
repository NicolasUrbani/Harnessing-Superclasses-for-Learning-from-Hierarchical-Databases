...
class TinyImagenetDataset(HFImageClassificationDataset):
    ...
    def __init__(self, path, transform=None, split=None, labels=None, reduction=None, seed=42):
        ...
        super().__init__(path, transform=transform, split=split, labels=None, reduction=reduction, seed=seed)
        self.labels = labels
...
class TinyImagenetSubsetDataModule(DataModule):
    ...
    def setup(self, stage=None):
        self.train_dataset = TinyImagenetDataset(
            self.datasets.path, transform=self.transforms.train, split="train", labels=self.labels, reduction=self.datasets.reduction_factor, seed=self.datasets.seed
        )
